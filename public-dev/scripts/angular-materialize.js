(function (angular) {
    ...
    */
        ...
        .directive("sidenav", [function () {
            return {
                ...
                link: function ($scope, $elem, $attrs) {
                    $elem.sidenav({
                        edge: $scope.edge ? $scope.edge : "left"
                    });

                    if('closeonclick' in $attrs){
    ...
    angular.module("ui.materialize.inputfield", [])
        .directive('inputField', ["$timeout", "$compile", function ($timeout, $compile) {
            ...
            return {
                ...
                link: function (scope, element) {
                    $timeout(function () {
                        ...
                        element.find('> input[type=password]').each(function(index, password){
                            element.prepend($compile('<div class="password-visibility waves-effect"><a><i class="material-icons">visibility</i></a></div>')(scope));
                            scope.$apply();
                        });
                        ...
                        element.find('> .materialize-textarea, > input').each(function (index, countable) {
                            ...
                            if (!countable.siblings('span[class="character-counter"]').length && countable.is("[character-counter]")) {
                                // countable.characterCounter();
                                M.CharacterCounter.init(countable);
                            }
                        });
                    }, 0);
                },
